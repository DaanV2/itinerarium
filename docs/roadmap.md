...
- [x] **A single API client.** `errorMessage`/`errorBody` are copy-pasted into 12 files under `lib/api/`, and the bearer-header + `fetchFn` plumbing repeats ~55 times. Extract `lib/api/client.ts` — one `apiFetch` that injects the token, parses the `{error, code}` body, and throws a typed `ApiError` (with `status` + `code`) that `DocumentConflictError` and friends extend. Each endpoint wrapper becomes a few lines. Done: `lib/api/client.ts` exposes `apiFetch` (parses the JSON body) and `apiSend` (for `204`/no-content deletes) over a shared request core; `DocumentConflictError` now extends `ApiError`. All 15 endpoint files were reduced to thin wrappers with no behavior change — the existing per-endpoint specs (which assert the exact `fetch` call shape) all still pass.
- [x] **Thin out the large route pages.** `routes/locations/[id]`, `routes/documents/[id]`, and `routes/repositories/import` (≈340–360 lines each) mix fetch orchestration, editor state, and markup. Extend the pattern the repo already uses well (`activity-view.ts`, `inventory-view.ts`, `document-reveal.ts`): pull the logic into tested `.ts` modules so the `.svelte` file is mostly template. Done: `document-editor.ts` (tag parsing, edit-form ↔ update-payload mapping, share-name resolution), `location-editor.ts` (grant labels, edit-form mapping), and `vault-import-view.ts` (repository labels, vault-relative path, result-row summary) each ship with a `*.spec.ts`; the pages now bind a single `form` object and call the helpers.
- [x] **Delete scaffolding.** Remove `lib/vitest-examples/` (the `greet` placeholder) now that real specs exist. Done (the `web/CLAUDE.md` testing pointer now references a real spec).
...
- [x] **Login/reset rate limiting.** `transport.Throttle` (in `transport/throttle.go`) throttles `/api/login` per client IP **and** per account with exponential backoff: past the `security.login-max-failures` threshold a key is locked for `security.login-lockout` (doubling per further failure up to 32×) and further attempts get `429` + `Retry-After`. A success clears the account's counter but not the IP's. The GM password-reset path is capped the same way, keyed by the target account (it is authenticated GM-only, so spam — not credential guessing — is the risk). The limiter is in-process and default-on; `security.login-max-failures: 0` disables it for deployments that throttle these paths at the reverse proxy instead. `security.trust-proxy-headers` opts into `X-Forwarded-For` for the client IP when behind a trusted proxy.
...
- [x] **Versioned migrations.** Schema evolution now runs through an ordered [gormigrate](https://github.com/go-gormigrate/gormigrate) list in `migrations.go`, applied automatically on start and recorded in a `migrations` table. `0001_initial_schema` is `AutoMigrate(allModels())` (the fresh-install fast path); `0002_backfill_activity_scopes` folds the former ad-hoc backfill into a numbered, idempotent step. There is no `InitSchema` shortcut, so a database predating gormigrate still runs every step once when it first adopts tracking. Documented in `architecture.md` (Schema migrations) and `deployment.md` (Upgrading).
- [ ] **An API contract to stop drift.** The ~50-route API is described only in prose in `architecture.md`, and the TypeScript types in `web/src/lib/types.ts` are hand-mirrored from the Go handlers. Publish an OpenAPI description (generated or maintained) and a check that the client types match it, so a handler change that outruns the client is caught in CI. _(Deferred — not scheduled this cycle; the hand-mirrored types are small and stable enough that the drift risk hasn't bitten. Revisit before the API surface grows materially.)_
