...
These live in `infrastructure/transport` (the app-agnostic HTTP mechanism package — middleware / handler decorators), wired in `components/router.go` and tuned by the `security.*` config set. See [deployment.md](deployment.md#security-hardening) and [SECURITY.md](../SECURITY.md).
...
- **Location visibility is grant-gated.** GMs see every location; a player sees one only through a `LocationAccess` grant held by one of their characters, directly or via a group that character belongs to. One grant is the single access level: view + modify, location fields, description, and inventory alike. Grants are GM-managed; players never see the grant list.
- **Location descriptions follow document rules.** A location's name, plane, and existence are gated only by `LocationAccess` (above); its description is additionally gated the way a document is: `Location.SharedOnGameDay` controls when the whole description becomes visible to a character who already has location access, and individual `LocationSection` rows flagged `gm_only` are stripped server-side for players — never sent, not even as an empty placeholder. `GET /api/locations/{id}` reports `revealed`: whether any character with access has reached `shared_on_game_day`, the same signal a document editor uses to warn about already-visible edits. Only a GM may change `shared_on_game_day` or mark a section `gm_only`; a player's section edit can only touch player-visible rows, and — exactly like documents — if every existing section is GM-only, the player's edit lands as a new player-visible section instead of failing (core domain rule 7).
