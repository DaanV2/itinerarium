...
func newDocumentsTransportEnv(t *testing.T) documentsTransportEnv {
	...
	router := transport.NewRouter(
		transport.WithHandle(
			"GET /api/repositories/{id}/documents", requireAuth(handlers.ListDocumentsHandler(docSvc)),
		),
		transport.WithHandle(
			"POST /api/repositories/{id}/documents", requireAuth(handlers.CreateDocumentHandler(docSvc)),
		),
		transport.WithHandle(
			"GET /api/repositories/{id}/documents/tree", requireAuth(handlers.GetDocumentFolderTreeHandler(docSvc)),
		),
		transport.WithHandle("GET /api/documents/shared", requireAuth(handlers.ListSharedDocumentsHandler(docSvc))),
		transport.WithHandle("GET /api/documents/{id}", requireAuth(handlers.GetDocumentHandler(docSvc))),
		transport.WithHandle("PATCH /api/documents/{id}", requireAuth(handlers.UpdateDocumentHandler(docSvc))),
		transport.WithHandle("POST /api/documents/{id}/share", requireAuth(handlers.ShareDocumentHandler(docSvc))),
		transport.WithHandle("GET /api/documents/{id}/shares", requireAuth(handlers.ListDocumentSharesHandler(docSvc))),
		transport.WithHandle(
			"POST /api/documents/{id}/shares", requireAuth(handlers.ShareDocumentWithCharacterHandler(docSvc)),
		),
		transport.WithHandle(
			"DELETE /api/documents/{id}/shares/{shareId}", requireAuth(handlers.RevokeDocumentShareHandler(docSvc)),
		),
		transport.WithHandle("GET /api/search", requireAuth(handlers.SearchDocumentsHandler(docSvc))),
		transport.WithHandle("POST /api/import/obsidian", requireAuth(handlers.ImportVaultHandler(vaultSvc))),
	)
