...
func newUsersTestEnv(t *testing.T, throttle ...*transport.Throttle) usersTestEnv {
	...
	router := transport.NewRouter(
		transport.WithHandle("GET /api/admin/users", requireAuth(handlers.ListAccountsHandler(userSvc))),
		transport.WithHandle("POST /api/admin/users", requireAuth(handlers.CreateAccountHandler(userSvc))),
		transport.WithHandle(
			"POST /api/admin/users/{id}/reset-password",
			requireAuth(handlers.ResetPasswordHandler(userSvc, resetThrottle)),
		),
	)
...
func TestResetPassword_ThrottlesRepeatedResetsPerTarget(t *testing.T) {
	now := time.Now()
	throttle := transport.NewLoginThrottle(2, time.Minute).WithClock(func() time.Time { return now })
