...
func newInventoryTestEnv(t *testing.T) inventoryTestEnv {
	...
	router := transport.NewRouter(
		transport.WithHandle("GET /api/currencies", requireAuth(handlers.ListCurrenciesHandler(catalogSvc))),
		transport.WithHandle("POST /api/currencies", requireAuth(handlers.CreateCurrencyHandler(catalogSvc))),
		transport.WithHandle("POST /api/currencies/convert", requireAuth(handlers.ConvertCurrencyHandler(catalogSvc))),
		transport.WithHandle("POST /api/currencies/simplify", requireAuth(handlers.SimplifyCurrencyHandler(catalogSvc))),
		transport.WithHandle("GET /api/items", requireAuth(handlers.ListItemDefinitionsHandler(catalogSvc))),
		transport.WithHandle("POST /api/items", requireAuth(handlers.CreateItemDefinitionHandler(catalogSvc))),
		transport.WithHandle(
			"GET /api/characters/{id}/inventory",
			requireAuth(handlers.ListInventoryHandler(inventorySvc, handlers.CharacterOwner)),
		),
		transport.WithHandle(
		...
		transport.WithHandle(
			...
			requireAuth(handlers.RemoveInventoryItemHandler(inventorySvc, handlers.CharacterOwner)),
		),
		transport.WithHandle(
			"GET /api/characters/{id}/money",
			requireAuth(handlers.ListMoneyHandler(inventorySvc, handlers.CharacterOwner)),
		),
		transport.WithHandle(
			"PUT /api/characters/{id}/money/{currencyId}",
			requireAuth(handlers.SetMoneyHandler(inventorySvc, handlers.CharacterOwner)),
		),
	)
