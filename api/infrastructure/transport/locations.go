...
func toLocationResponse(l *models.Location) locationResponse {
	return locationResponse{
		ID: l.ID, Name: l.Name, Description: l.Description, ParentID: l.ParentID,
	}
}

// ListLocationsHandler returns every location. Any authenticated caller may
// read them. Must be wrapped in RequireAuth.
...
// CreateLocationHandler adds a location (GM only). Must be wrapped in
// RequireAuth.
func CreateLocationHandler(svc *application.LocationService) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		l, err := svc.Create(r.Context(), requesterFrom(r), req.Name, req.Description, req.ParentID)
		if err != nil {
			writeLocationServiceError(w, err)
...
// UpdateLocationHandler edits a location's name and/or description (GM only).
...
func UpdateLocationHandler(svc *application.LocationService) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		l, err := svc.Update(r.Context(), requesterFrom(r), r.PathValue("id"), req.Name, req.Description)
		if err != nil {
			writeLocationServiceError(w, err)
...
// DeleteLocationHandler removes a location (GM only). Must be wrapped in
// RequireAuth.
func DeleteLocationHandler(svc *application.LocationService) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		err := svc.Delete(r.Context(), requesterFrom(r), r.PathValue("id"))
		if err != nil {
			writeLocationServiceError(w, err)
...
func writeLocationServiceError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, application.ErrLocationHasChildren):
		writeError(w, http.StatusConflict, err.Error())
	case errors.Is(err, application.ErrInvalidName), errors.Is(err, application.ErrUnknownParent):
