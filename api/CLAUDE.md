...
- **Database backends**: selected by `database.type` (`sqlite` default, `memory`, `postgres`, `mysql`). `persistence.New` picks the GORM dialector; `persistence.GetOptions`/`DatabaseConfig.Options` (`persistence/config.go`) validate the config (postgres/mysql require `--database.dsn`) and translate it into options. Adding a backend = a new `DBType` const + a case in `settings.dialector`. All drivers must stay pure Go (no cgo) or the static Docker build breaks.
