...
- **Config**: read via `config.GetContext("component")`; bind command flags with `config.MustBindFlags` (whole flag set) or `config.MustBindFlag` (single flag, e.g. the shared `cmd/database_flags.go`). Key `server.database-path` = flag `--database-path` = env `SERVER_DATABASE_PATH` = YAML `server.database-path`.
...
- **Database backends**: selected by `server.database-type` (`sqlite` default, `memory`, `postgres`, `mysql`). `persistence.New` picks the GORM dialector; `components.SetupDatabase`/`databaseOptions` validate the config (postgres/mysql require `--database-dsn`) and translate it into options. Adding a backend = a new `DBType` const + a case in `settings.dialector`. All drivers must stay pure Go (no cgo) or the static Docker build breaks.
- **SQLite driver**: `github.com/glebarez/sqlite` (pure Go, no cgo). Do not switch to `gorm.io/driver/sqlite` — it needs cgo. Postgres uses `gorm.io/driver/postgres` (pgx), MySQL uses `gorm.io/driver/mysql` — both pure Go.
- **Embedded web UI**: `infrastructure/webapp` embeds the SvelteKit build (written into `webapp/dist` by `just web-build`) behind the `embedweb` build tag; `transport.SPAHandler` serves it with an SPA fallback and keeps unmatched `/api` paths as 404s. Plain `go build ./...` (dev, CI) compiles the no-embed stub and serves the API only — `just build` and the root `Dockerfile` pass `-tags embedweb`, so the tag never makes the ordinary toolchain depend on a Node build.
